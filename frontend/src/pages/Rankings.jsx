  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Candidate rankings by drive</p>
      </div>
      ...
      <div className="relative">
        <button
          ...
          className="glass w-full flex items-center justify-between px-5 py-4 hover:border-[var(--border-hover)] transition-all"
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>Viewing rankings for</p>
              <p className="font-display font-semibold" style={{ color: 'var(--text-primary)' }}>
                {selectedDriveName || 'Select a drive...'}
              </p>
            </div>
          </div>
          <ChevronDown size={18} style={{ color: 'var(--text-muted)', transform: driveOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
        </button>
        ...
        {driveOpen && (
          ...
          >
            {drives.length === 0 ? (
              <div className="px-5 py-4 text-sm" style={{ color: 'var(--text-muted)' }}>No drives found</div>
            ) : drives.map((d) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{d.company || 'No company'} · {d.totalCandidates || 0} candidates</p>
                </div>
                {selectedDrive === d._id && (
      ...
      {loading ? (
        ...
        <div className="glass p-16 text-center">
          <Trophy size={48} className="mx-auto mb-4 opacity-20" style={{ color: 'var(--text-muted)' }} />
          <p style={{ color: 'var(--text-muted)' }}>No ranked candidates yet for this drive</p>
        </div>
      ) : (
        <div className="glass overflow-hidden">
          ...
          {rankings.length >= 3 && (
            <div className="p-8" style={{ borderBottom: '1px solid var(--border)', background: 'rgba(99,114,241,0.03)' }}>
              <p className="text-center text-xs font-semibold uppercase tracking-widest mb-6" style={{ color: 'var(--text-muted)' }}>Top Performers</p>
                  ...
                  return (
                    <motion.div key={c._id}
                      ...
                      <div className="w-20 rounded-t-xl flex items-start justify-center pt-2"
                        style={{ height: heights[pos === 0 ? 1 : pos === 1 ? 0 : 2] * 4, background: isFirst ? `linear-gradient(to bottom, rgba(245,158,11,0.25), rgba(245,158,11,0.05))` : 'var(--bg-card)' }}>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold"
                          style={{ background: ms ? ms.bg : 'rgba(255,255,255,0.04)', color: ms ? ms.color : 'var(--text-muted)', outline: ms ? ms.ring : undefined }}>
                          {i + 1}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <Link to={`/candidates/${c._id}`}>
                          ...
                          >{c.name}</p>
                          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{c.college || c.email}</p>
                        </Link>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
