    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Admin-only · Manage all user accounts and permissions</p>
        </div>
        ...
        <div className="flex gap-2">
          {[['', 'All'], ['admin', '👑 Admins'], ['recruiter', '🎯 Recruiters']].map(([val, label]) => (
            <button key={val} onClick={() => setRoleFilter(val)}
              ...
              style={{
                background: roleFilter === val ? 'rgba(99,114,241,0.15)' : 'var(--bg-card)',
                border: `1px solid ${roleFilter === val ? 'rgba(99,114,241,0.4)' : 'var(--border)'}`,
                color: roleFilter === val ? 'var(--accent)' : 'var(--text-secondary)',
              }}>{label}
            </button>
          ))}
        </div>
      ...
      <div className="glass overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            <Users size={36} className="mx-auto mb-3 opacity-20" style={{ color: 'var(--text-muted)' }} />
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold text-white shrink-0"
                        style={{ background: user.role === 'admin' ? 'linear-gradient(135deg,#f59e0b,#f97316)' : 'linear-gradient(135deg,var(--accent),var(--accent-cyan))' }}>
