        ...
        <div className="space-y-4">
          ...
          <div className="glass p-5">
            ...
            <div className="relative">
              <button onClick={() => setDriveOpen(!driveOpen)}
                ...
                <span style={{ color: selectedDrive ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                  {selectedDriveName || 'Select a drive for requirement matching...'}
                </span>
                <ChevronDown size={15} style={{ color: 'var(--text-muted)', transform: driveOpen ? 'rotate(180deg)' : 'none', transition: '0.2s' }} />
              </button>
              <AnimatePresence>
                {driveOpen && (
                  <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
                    className="absolute top-full left-0 right-0 mt-1 rounded-xl overflow-hidden z-20 shadow-2xl"
                    style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', maxHeight: 240, overflowY: 'auto' }}>
          ...
          <div className="glass p-5">
            ...
            {!file ? (
              ...
              >
                <Upload size={36} className="mx-auto mb-3" style={{ color: dragging ? 'var(--accent)' : 'var(--text-muted)', opacity: 0.6 }} />
              ...
              <div className="flex items-center gap-4 p-4 rounded-xl"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{(file.size / 1024).toFixed(1)} KB · {file.type}</p>
                </div>
          ...
          {!result ? (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              ...
              <div className="glass p-6 text-center">
                ...
                <p className="text-sm mt-3 max-w-xs mx-auto" style={{ color: 'var(--text-muted)' }}>{sc.msg}</p>
              </div>

              {/* Breakdown */}
              <div className="glass p-5 space-y-3">
                <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Score Breakdown</p>
              ...
              <div className="glass p-5 space-y-4">
                <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>Parsed Details</p>
                ...
                {result.extractedSkills?.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold mb-2" style={{ color: 'var(--text-muted)' }}>Detected Skills ({result.extractedSkills.length})</p>
