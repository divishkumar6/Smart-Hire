...
const PieTT = ({ active, payload }) => {
  ...
  const p = payload[0];
  return (
    <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 10, padding: '8px 14px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ width: 10, height: 10, borderRadius: 3, background: p.payload.fill }} />
        <span style={{ color: 'var(--text-primary)', fontWeight: 600, textTransform: 'capitalize', fontSize: 13 }}>{p.name?.replace('_', ' ')}</span>
      </div>
          ...
          <div className="glass p-6">
              ...
              {drive.rounds?.map((round, i) => (
                <div key={round._id} className="flex items-center gap-3 p-3 rounded-xl"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{round.name}</p>
                    <p className="text-xs capitalize" style={{ color: 'var(--text-muted)' }}>{round.type?.replace('_', ' ')}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-sm font-bold" style={{ color: roundColors[i % roundColors.length] }}>{round.weightage}%</p>
                    <p className="text-xs" style={{ color: 'var(--text-muted)' }}>cut: {round.cutoffScore}</p>
                  </div>
                </div>
              ))}
              {(!drive.rounds || drive.rounds.length === 0) && (
                <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>No rounds configured</p>
              )}
            </div>
            {drive.rounds?.length > 0 && (
              <div className="mt-4 pt-4" style={{ borderTop: '1px solid var(--border)' }}>
                ...
                <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>Weightage distribution</p>
              </div>
            )}
          </div>
                ...
                {pieData.map(item => (
                  <div key={item.name} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="capitalize" style={{ color: 'var(--text-secondary)' }}>{item.name?.replace('_', ' ')}</span>
                    </div>
          ...
          {candidates.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <div className="text-center">
                <p className="font-semibold" style={{ color: 'var(--text-secondary)' }}>No candidates yet</p>
                <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Add candidates to start evaluating</p>
              </div>
              <Link to={'/drives/' + id + '/add-candidates'} className="btn-primary flex items-center gap-2 text-sm">
                <Plus size={14} /> Add Candidates
              </Link>
                  ...
                  <motion.div key={c._id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold shrink-0"
                        style={{
                          background: isMedal ? medalColors[i] + '20' : 'var(--bg-card)',
                          color: isMedal ? medalColors[i] : 'var(--text-muted)',
