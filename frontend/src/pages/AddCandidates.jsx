  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between p-4 rounded-xl"
        ...
        <div>
          <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Download Template First</p>
      ...
      {!file ? (
        ...
        >
          ...
          <p className="font-semibold text-sm" style={{ color: 'var(--text-secondary)' }}>Drop .xlsx file or click to browse</p>
          <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>Supports the SmartHire template format · Max 500 rows</p>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 p-4 rounded-xl"
            ...
            <div className="flex-1">
              ...
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{file.rows?.length} rows detected</p>
            </div>
                    ...
                    {preview.map((row, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                        {['name', 'email', 'usn', 'college', 'branch'].map(k => (
                          <td key={k} className="px-3 py-2 truncate max-w-xs" style={{ color: 'var(--text-secondary)' }}>
                            {row[k] || '—'}
                          </td>
                        ))}
                      </tr>
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
            {candidates.map(c => (
              <div key={c._id}
                ...
                style={{
                  background: selected.has(c._id) ? 'rgba(99,114,241,0.1)' : 'var(--bg-card)',
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          {drive && <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>to {drive.name}</p>}
        </div>
      </div>
