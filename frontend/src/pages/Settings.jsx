  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {themeOptions.map(({ value, icon: Icon, label }) => (
            <button key={value} onClick={() => changeTheme(value)}
              ...
              style={{
                background: theme === value ? 'rgba(99,114,241,0.12)' : 'var(--bg-card)',
                border: `1px solid ${theme === value ? 'rgba(99,114,241,0.4)' : 'var(--border)'}`,
              }}>
              <Icon size={20} style={{ color: theme === value ? 'var(--accent)' : 'var(--text-muted)' }} />
              <span className="text-xs font-medium" style={{ color: theme === value ? 'var(--accent)' : 'var(--text-secondary)' }}>{label}</span>
      ...
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass p-6">
        ...
        <div className="flex items-center gap-4 mb-6 p-4 rounded-xl" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          ...
          <div>
            <p className="font-display font-semibold" style={{ color: 'var(--text-primary)' }}>{user?.name}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass p-6">
        ...
        <div className="space-y-3 text-sm">
          {[
            ['Account ID', <span className="font-mono text-xs" style={{ color: 'var(--text-muted)' }}>{user?._id}</span>],
            ['Role', <span style={{ color: 'var(--text-primary)' }} className="capitalize">{user?.role}</span>],
            ['Last Login', <span style={{ color: 'var(--text-muted)' }}>{user?.lastLogin ? new Date(user.lastLogin).toLocaleDateString() : 'N/A'}</span>],
          ].map(([label, val]) => (
